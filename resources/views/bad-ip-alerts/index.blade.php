...
<div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
    ...
    <div class="glass-panel p-5 rounded-2xl border border-white/5 flex items-center justify-between group hover:bg-slate-800/50 transition-colors">
        <div>
            ...
            <p class="text-3xl font-black text-blue-400 leading-none">{{ number_format($stats['new']) }}</p>
        </div>
    ...
    <div class="glass-panel p-5 rounded-2xl border border-white/5 flex items-center justify-between group hover:bg-slate-800/50 transition-colors">
        <div>
            ...
            <p class="text-3xl font-black text-amber-400 leading-none">{{ number_format($stats['acknowledged']) }}</p>
        </div>
    ...
    <div class="glass-panel p-5 rounded-2xl border border-white/5 flex items-center justify-between group hover:bg-slate-800/50 transition-colors">
        <div>
            ...
            <p class="text-3xl font-black text-emerald-400 leading-none">{{ number_format($stats['resolved']) }}</p>
        </div>
                    ...
                    @forelse($alerts as $alert)
                        <tr class="hover:bg-white/[0.02] transition-colors group">
                            <td class="px-4 py-3 text-center">
                                <input type="checkbox" name="alert_ids[]" value="{{ $alert->id }}" class="alert-checkbox w-4 h-4 rounded border-slate-700 bg-slate-800 text-indigo-500 focus:ring-indigo-500 focus:ring-offset-slate-900" {{ $alert->status === 'Resolved' ? 'disabled' : '' }}>
                            </td>
                            <!-- Severity -->
                            <td class="px-5 py-3">
                                @php $color = $alert->severity_color; @endphp
                                <span class="px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider bg-{{ $color }}-500/20 text-{{ $color }}-400 border border-{{ $color }}-500/30">
                                    {{ $alert->signature_severity ?? 'N/A' }}
                                </span>
                            </td>
                            <!-- Rule -->
                            ...
                            <!-- Source IP -->
                            <td class="px-5 py-3">
                                <span class="px-2 py-1 rounded bg-rose-500/10 text-rose-400 border border-rose-500/20 text-xs font-mono">
                                    {{ $alert->src_ip }}
                                </span>
                            </td>
                            <!-- Destination -->
                            <td class="px-5 py-3">
                                @if($alert->dest_ip)
                                    <div class="text-xs font-mono text-slate-300">{{ $alert->dest_ip }}{{ $alert->dest_port ? ':' . $alert->dest_port : '' }}</div>
                                    <div class="text-[10px] text-slate-500 uppercase mt-0.5">{{ $alert->proto }}</div>
                                @else
                            ...
                            <td class="px-5 py-3 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    @if($alert->status !== 'Resolved')
                                        <form method="POST" action="{{ route('bad-ip-alerts.update-status', $alert->id) }}" class="inline-flex">
                                            @csrf
                                            @method('PATCH')
                                            <input type="hidden" name="status" value="Resolved">
                                            <button type="submit" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 hover:text-emerald-300 rounded-lg transition-colors border border-emerald-500/20 hover:border-emerald-500/30 text-xs font-medium content-['Resolve']">
                                                Resolve
                                            </button>
                                        </form>
                                    @endif
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Initial setup in case browser cached checked state on reload
        updateBulkActions();
