...
sys.stderr.write(f"DEBUG: Executable: {sys.executable}\n")
...
import fnmatch
try:
    import pefile    # optional
except ImportError:
    pass

# FALLBACK: Add user site-packages if not present (Fix for Web Server Context)
try:
    import site
    user_site = site.getusersitepackages()
    if user_site not in sys.path:
        sys.stderr.write(f"DEBUG: Appending user site-packages: {user_site}\n")
        sys.path.append(user_site)
except:
    pass

# Also try generic expansion just in case
try:
    import site
    user_site = site.getusersitepackages()
    if user_site not in sys.path:
